...
import { Esp32View } from './components/Esp32View';
...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    if (currentView === View.LIVE_TIMING && selectedEventId) return <LiveTimingView eventId={selectedEventId} onBack={handleBackToEvents} onDataUpdate={refreshData} swimmers={swimmers} competitionInfo={competitionInfo} onStatusChange={handleArduinoStatusChange} onOpenEsp32Menu={() => navigateTo(View.ESP32_SETTINGS)} />;
    ...
    switch (currentView) {
      ...
      case View.ESP32_SETTINGS: return <Esp32View navigateTo={navigateTo} />;
      ...
      <aside className={`w-64 bg-surface p-4 flex flex-col justify-between fixed inset-y-0 left-0 z-40 transform ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out md:relative md:translate-x-0 no-print`}>
        <div>
            ...
            <nav className="space-y-2">
                ...
                <NavLink label="Koneksi ESP32" icon={<UsbIcon />} isActive={currentView === View.ESP32_SETTINGS} onClick={() => navigateTo(View.ESP32_SETTINGS)}/>
        ...
        <div className="space-y-2">
            <ConnectionStatusIndicator internetStatus={internetStatus} dbStatus={dbStatus} arduinoStatus={arduinoStatus} onOpenEsp32={() => navigateTo(View.ESP32_SETTINGS)} />
            <ThemeToggle />
